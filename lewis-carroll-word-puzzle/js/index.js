...
buttonElem.addEventListener('click', async () => {
    ...
    if (changedWord.length === selectedWord.start.length) {
        ...
        for (let index = 0; index < lettersChangedWord.length; index++) {
            if (lettersChangedWord[index] !== lastWordArray[index]) { // Kolla om varje bokstav är olik eller är samma
        ...
        if (changedLetters === 1) {
            ...
            const response = await fetch(`https://api.dictionaryapi.dev/api/v2/entries/en/${changedWord}`);
